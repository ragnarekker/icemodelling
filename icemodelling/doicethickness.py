...
from icemodelling import doenergybalance as deb
            ...
            while time_step > 0 and len(ic.column) > 0:
                if ic.column[0].type == 'water':
                    ...
                    if ic.column[0].get_enum() >= 20: # snow
                        ...
                        ml.log_and_print('doicethickness.py -> get_ice_thickness_from_surface_temp: '
                                         'Melting on unknown layer type: {0}. Using slush_ice coeff.'.format(ic.column[0].type))
                        meltingcoeff = const.meltingcoeff_slush_ice
